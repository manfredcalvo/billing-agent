# Billing Agent (Short-Term Memory)

A standalone LangGraph billing-support agent with short-term memory (Lakebase) and a built-in
chat UI. It deploys as a single Databricks App via the bundle in `databricks.yml`. The agent
API (FastAPI) owns the app port and serves `/invocations` and `/responses`, while the built-in
chat UI runs internally and is reverse-proxied by the agent server
(`AgentServer(enable_chat_proxy=True)` in `agent_server/start_server.py`). So **both the API
and the chat UI are available at the app URL** — `/responses` for programmatic calls, `/` for
the UI.
...
The agent implements the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses) interface. It is a call-center billing assistant with three tools defined in `agent_server/tools.py` — `get_customer_key`, `list_invoices`, and `get_invoice_info` — which currently run on mock data (`USE_MOCK_DATA=true`) and can be wired to Lakebase. You can customize agent code and test it via the API or UI.
...
3. **Create and link an MLflow experiment to your app**
   ...
   databricks experiments create-experiment /Users/$DATABRICKS_USERNAME/billing-agent
   ```
...
This project uses [Databricks Asset Bundles (DABs)](https://docs.databricks.com/aws/en/dev-tools/bundles/) for deployment. The `databricks.yml` file defines a single app (`billing_agent`), a Lakebase autoscaling project, and an MLflow experiment.
...
2. **Deploy the bundle**
   ...
   This uploads your code and creates resources (MLflow experiment, Lakebase project, app) defined in `databricks.yml`:

   ```bash
   databricks bundle deploy
   ```

3. **Set the Lakebase database ID and redeploy**

   The Lakebase project is created with an auto-generated database ID on the first deploy. Retrieve it (the resource suffix for the dev target is `dev-<your-username>`):

   ```bash
   databricks postgres list-databases \
     "projects/billing-db-dev-<your-username>/branches/production" \
     --output json | jq -r '.[0].name'
   ```

   The value looks like `db-xxxx-xxxxxxxxxxxx`. Set it as the `lakebase_database_id` default in `databricks.yml`, then redeploy:

   ```yaml
   variables:
     lakebase_database_id:
       default: "db-xxxx-xxxxxxxxxxxx"   # ← value from the command above
   ```
   ...
   > **Note:** This only needs to be done once per Lakebase project.

4. **Start the app**

   ```bash
   databricks bundle run billing_agent
   ```
   ...
   >
   > **Cold start:** Because the deployed app serves the built-in chat UI, the first start clones and builds the Next.js frontend (`npm install && next build`) — expect a 3-5 minute cold start. Subsequent restarts are faster.
...
5. **Grant Lakebase permissions to your App's Service Principal**
   ...
   After deploying, the app's service principal needs access to the Lakebase tables used for memory. Use the role setup script to create a Postgres role for the SP and grant schema-level permissions:

   ```bash
   # Get the SP client ID from your deployed app
   SP_CLIENT_ID=$(databricks apps get billing-agent --profile <profile> --output json | jq -r '.service_principal_client_id')

   # Run the role setup script (project ID is billing-db-dev-<your-username> for the dev target)
   uv run python scripts/lakebase-role-setup.py \
     --profile <profile> \
     --project-id billing-db-dev-<your-username> \
     --sp-client-id $SP_CLIENT_ID
   ```

   This script will:
   - Create a Postgres role for the service principal (idempotent)
   - Grant CONNECT, CREATE, USAGE, and table privileges on the `public` schema
   - Set default privileges so future tables created by the app are accessible

   Then open the app URL — the built-in chat UI is served directly from the app:

   ```bash
   databricks apps get billing-agent --profile <profile> --output json | jq -r '.url'
   ```
...
For future updates, run `databricks bundle deploy` and `databricks bundle run billing_agent` to redeploy.
...
- **`databricks bundle deploy` fails with "An app with the same name already exists"**
  ...
  databricks bundle deployment bind billing_agent <app-name> --auto-approve
...
- **App is running old code after `databricks bundle deploy`**
  ...
  `bundle deploy` only uploads files and configures resources. You must run `databricks bundle run billing_agent` to actually start/restart the app with the new code.
