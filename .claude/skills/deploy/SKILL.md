...
The resource key `billing_agent` matches the app name in `databricks.yml` under `resources.apps`.
...
databricks bundle deployment bind billing_agent <existing-app-name> --auto-approve
```
...
```bash
databricks bundle deployment unbind billing_agent
```
...
| App running old code after deploy | Run `databricks bundle run billing_agent` after deploy |
